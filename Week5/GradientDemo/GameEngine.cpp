//-----------------------------------------------------------------
// Game Engine Object
// C++ Source - GameEngine.cpp - version v7_02_firstyear
//-----------------------------------------------------------------
...
//-----------------------------------------------------------------
// String conversion macros for firstyear engine (this is a good example of where macros are useful)
//-----------------------------------------------------------------
#define TO_TSTRING(textVar) tstring(textVar.cbegin(), textVar.cend())
#define TO_STDSTRING(textVar) std::string(textVar.cbegin(), textVar.cend())
...
//-----------------------------------------------------------------
void GameEngine::SetTitle(const std::string& titleRef)
{
	m_Title = TO_TSTRING(titleRef);
}
...
bool GameEngine::SetGameValues(const std::string& titleRef, WORD icon, WORD smallIcon, int width = 640, int height = 480)
{
	SetTitle(titleRef);
	SetIcon(icon);
...
void GameEngine::SetKeyList(const std::string& keyListRef)
{
	delete m_KeyListPtr; // clear list if one already exists

	// make keylist if needed
	if (keyListRef.size() > 0)
	{
		tstring tstringList(keyListRef.begin(), keyListRef.end());
		...
		{
			TCHAR key = tstringList.c_str()[count];
...
bool GameEngine::MessageContinue(const std::string& textRef) const
{
	// MessageBox define is undef'd at begin of GameEngine.h
	#ifdef UNICODE						
		return MessageBoxW(GetWindow(), TO_TSTRING(textRef).c_str(), TO_TSTRING(m_Title).c_str(), MB_ICONWARNING | MB_OKCANCEL) == IDOK;
	#else
		return MessageBoxA(GetWindow(), textRef.c_str(), m_Title.c_str(), MB_ICONWARNING | MB_OKCANCEL) == IDOK;
	#endif 
}

void GameEngine::MessageBox(const std::string& textRef) const
{
	// MessageBox define is undef'd at begin of GameEngine.h
	#ifdef UNICODE						
		MessageBoxW(GetWindow(), TO_TSTRING(textRef).c_str(), TO_TSTRING(m_Title).c_str(), MB_ICONEXCLAMATION | MB_OK);
	#else
		MessageBoxA(GetWindow(), textRef.c_str(), m_Title.c_str(), MB_ICONEXCLAMATION | MB_OK);
	#endif 
}
...
SIZE GameEngine::CalculateTextDimensions(const std::string& textRef, Font* fontPtr) const
{
	SIZE size;
	HDC hdc = GetDC(NULL);
	SelectObject(hdc, fontPtr->GetHandle());    //attach font to hdc

	GetTextExtentPoint32(hdc, TO_TSTRING(textRef).c_str(), (int) textRef.size(), &size);
...
SIZE GameEngine::CalculateTextDimensions(const std::string& textRef, Font* fontPtr, RECT rect) const
{
	SIZE size;
	HDC hdc = GetDC(NULL);
	SelectObject(hdc, fontPtr->GetHandle());    //attach font to hdc
	...
	int height = DrawText(hdc, TO_TSTRING(textRef).c_str(), (int) textRef.size(), &rect, DT_CALCRECT);
...
{
	if (m_IsDoublebuffering || m_IsPainting)
	{
...
int GameEngine::DrawString(const std::string& textRef, int x, int y, int width, int height) const
{
	if (m_IsDoublebuffering || m_IsPainting)
	{
		...
		if (m_FontDraw != 0)
		{
			hOldFont = (HFONT)SelectObject(m_HdcDraw, m_FontDraw);

			oldColor = SetTextColor(m_HdcDraw, m_ColDraw);
			SetBkMode(m_HdcDraw, TRANSPARENT);

			RECT rc = { x, y, x + width - 1, y + height - 1 };
			int result = DrawText(m_HdcDraw, TO_TSTRING(textRef).c_str(), -1, &rc, DT_WORDBREAK);
...
int GameEngine::DrawString(const std::string& textRef, int x, int y) const
{
	if (m_IsDoublebuffering || m_IsPainting)
	{
		HFONT hOldFont;
		...
		if (m_FontDraw != 0)
		{
			...
			int result = TextOut(m_HdcDraw, x, y, TO_TSTRING(textRef).c_str(), (int)textRef.size());
...
bool GameEngine::DrawSolidBackground(COLORREF color, RECT rect)
{
	if (m_IsDoublebuffering || m_IsPainting)
	{
		COLORREF oldColor = GetDrawColor();
		SetColor(color);
		FillRect(0, 0, rect.right, rect.bottom);
		SetColor(oldColor);
...
{	
	return DrawSolidBackground(color, m_RectDraw);
}
...
std::string	GameEngine::GetTitle() const
{
	#pragma warning(disable:4244)
	return TO_STDSTRING(m_Title);
...
Bitmap::Bitmap(const std::string& nameRef, bool createAlphaChannel) : m_hBitmap(0), m_TransparencyKey(-1), m_Opacity(100), m_PixelsPtr(0), m_Exists(false)
{
	// check if the file to load is a targa
	size_t len = nameRef.length(); 

	if (len > 4 && TO_TSTRING(nameRef).substr(len-4) == _T(".tga"))
	{
		m_IsTarga = true;
		m_HasAlphaChannel = true;
		TargaLoader* targa = new TargaLoader();

		if (targa->Load((TCHAR*) TO_TSTRING(nameRef).c_str()) == 1)
		{
	...
	else 
	{
		...
		m_hBitmap = (HBITMAP)LoadImage(GameEngine::GetSingleton()->GetInstance(), TO_TSTRING(nameRef).c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		if (m_hBitmap != 0) m_Exists = true;
	}
...
Bitmap::Bitmap(int IDBitmap, const std::string& typeRef, bool createAlphaChannel): m_TransparencyKey(-1), m_Opacity(100), m_Exists(false)
{
	tstring tstringType{ TO_TSTRING(typeRef) };

	if (tstringType == _T("BITMAP"))
	{	
		m_IsTarga = false;
		m_HasAlphaChannel = createAlphaChannel;
...
bool Bitmap::SaveToFile(const std::string& fileNameRef) const
{
	bool result = true;
	...
	HANDLE hFile = ::CreateFile(TO_TSTRING(fileNameRef).c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
Audio::Audio(const std::string& nameRef) : m_Playing(false), m_Paused(false), m_MustRepeat(false), m_hWnd(0), m_Volume(100), m_Duration(-1)
{	
	tstring tstringName{ TO_TSTRING(nameRef) };

	size_t len = tstringName.length();

	ASSERT(len >= 4, _T("Audio name length must be longer than 4 characters!"));
	if (len < 4) return;

	tstring end = tstringName.substr(len - 4);
	...
	if (end == _T(".mp3") || end == _T(".wav") || end == _T(".mid"))
	{
		...
		m_Alias = buffer.str();
		m_FileName = tstringName;

		Create(tstringName);
	}
}

Audio::Audio(int IDAudio, const std::string& typeRef) : m_Playing(false), m_Paused(false), m_MustRepeat(false), m_hWnd(0), m_Volume(100), m_Duration(-1)
{
	tstring tstringType{ TO_TSTRING(typeRef) };

	if (tstringType == _T("MP3") || tstringType == _T("WAV") || tstringType == _T("MID"))
	{
		tstringstream buffer;
		buffer << _T("audio");
		buffer << m_Nr++;
		...
		if (tstringType == _T("MP3")) m_FileName += _T(".mp3");
		else if (tstringType == _T("WAV")) m_FileName += _T(".wav");
...
{
	return TO_STDSTRING(m_FileName);
}
	
std::string Audio::GetAlias() const
{
	return TO_STDSTRING(m_Alias);
}
...
#pragma warning(disable:4311)	
#pragma warning(disable:4312)
TextBox::TextBox(const std::string& textRef) : m_X(0), m_Y(0), m_BgColor(RGB(255, 255, 255)), m_ForeColor(RGB(0, 0, 0)), m_BgColorBrush(0), m_Font(0), m_OldFont(0)
{
	// Create the edit box
	m_hWndEdit = CreateWindow(_T("EDIT"), TO_TSTRING(textRef).c_str(), WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | ES_LEFT | ES_AUTOHSCROLL, 0, 0, 0, 0, GameEngine::GetSingleton()->GetWindow(), NULL, GameEngine::GetSingleton()->GetInstance(), nullptr);
...
std::string TextBox::GetText() const
{
	...
	return TO_STDSTRING(newString);
}

void TextBox::SetText(const std::string& textRef)
{
	SendMessage(m_hWndEdit, WM_SETTEXT, 0, (LPARAM) TO_TSTRING(textRef).c_str());
}

void TextBox::SetFont(const std::string& fontNameRef, bool bold, bool italic, bool underline, int size)
{
	LOGFONT ft;

	tstring tstringFontName{ TO_TSTRING(fontNameRef) };
	...
	{
		ft.lfFaceName[teller] = tstringFontName[teller];
	}
...
#pragma warning(disable:4311)
#pragma warning(disable:4312)
Button::Button(const std::string& textRef) : m_X(0), m_Y(0), m_Armed(false), m_Font(0), m_OldFont(0)
{
	// Create the button object
	m_hWndButton = CreateWindow(_T("BUTTON"), TO_TSTRING(textRef).c_str(), WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | BS_PUSHBUTTON, 0, 0, 0, 0, GameEngine::GetSingleton()->GetWindow(), nullptr, GameEngine::GetSingleton()->GetInstance(), NULL);
...
void Button::SetText(const std::string& textRef)
{
	SendMessage(m_hWndButton, WM_SETTEXT, 0, (LPARAM) TO_TSTRING(textRef).c_str());
}

void Button::SetFont(const std::string& fontNameRef, bool bold, bool italic, bool underline, int size)
{
	LOGFONT ft;
...
Font::Font(const std::string& fontNameRef, bool bold, bool italic, bool underline, int size)
{
	tstring tstringFontName{ TO_TSTRING(fontNameRef) };
...
{
	OutputDebugString(TO_TSTRING(textRef).c_str());
}
