...
#pragma region CollisionFunctionality
	...
	{
		const bool inHorizontal{point.x >= rectangle.left && point.x <= rectangle.left + rectangle.width};
		const bool inVertical{point.y >= rectangle.top && point.y <= rectangle.top + rectangle.height};
	...
	{
		if (rect2.top > rect1.top + rect1.height) return false; // second below first
		if (rect1.top > rect2.top + rect2.height) return false; // first below second
		if (rect2.left > rect1.left + rect1.width) return false; // second right of first
		if (rect1.left > rect2.left + rect2.width) return false; // first right of second
